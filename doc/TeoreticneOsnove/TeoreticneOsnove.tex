...
Ko imamo za vsak cm višine gladine vode izračunan omočeni obod $P_n(h)$ odseka $n$ in ploščino prečnega prereza pod gladino vode $S_n(h)$, lahko določimo pretok vode skozi odsek struge vodotoka $Q(h)$. Pretok vode skozi odsek izračunamo po Manningovi enačbi opisani v poglavju \ref{sec:teorija_trapeznaMetoda}. 

%before:Za vsak računani odsek $m$ moramo poznati tudi naklon struge vodotoka $I_m$ in  Manningov koeficient hrapavosti površine $ng_m$, ki se ju določi po postopkih opisanih v poglavju \ref{sec:teorija_pridobitevPodatkov}.
...
Posamezne ploščine odsekov $S_n$ po višinah medsebojno seštejemo in dobimo končno površino vodnega telesa v odvisnosti od višine gladine vode v strugi vodotoka, ter po enakem postopku določimo omočeni obod v prečni smeri $P_n$, kar je shematično prikazano na sliki ~\ref{fig:customChannel_sestevek}:


%before: Posamezne pretoke odsekov po višinah medsebojno seštejemo in dobimo končne vrednosti pretokov $Q$ v odvisnosti od višine gladine vode v strugi vodotoka, kar je shematično prikazano na sliki~\ref{fig:customChannel_sestevek}:
